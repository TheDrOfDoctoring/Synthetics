...
public class SyntheticsDataComponents {
    ...
    public static final Supplier<DataComponentType<SyntheticAugment>> AUGMENT = DATA_COMPONENTS.registerComponentType(
            SyntheticsData.AUGMENTS.location().getPath(),
            builder -> builder
                    .persistent(SyntheticAugment.CODEC.codec())
                    .networkSynchronized(SyntheticAugment.STREAM_CODEC)
    );
    public static final Supplier<DataComponentType<BodyPart>> BODY_PART = DATA_COMPONENTS.registerComponentType(
            SyntheticsData.BODY_PARTS.location().getPath(),
            builder -> builder
                    .persistent(BodyPart.CODEC.codec())
                    .networkSynchronized(BodyPart.STREAM_CODEC)
    );
}
